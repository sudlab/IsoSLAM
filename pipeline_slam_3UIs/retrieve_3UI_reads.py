...
def main(argv=None):
    ...
    for pair in fragment_iterator(bamfile.fetch(until_eof=True)):
        ...
        assign_conversions_to_retained = \
            list(read1_within_intron) + list(read2_within_intron)

        assign_conversions_to_retained = [element
                                          for element 
                                          in assign_conversions_to_retained]

        assign_conversions_to_spliced = \
            list(read1_spliced_3UI.keys()) + list(read2_spliced_3UI.keys())
            
        assign_conversions_to_spliced = \
            list(set(assign_conversions_to_spliced))
            
        assign_conversions_to_spliced = [element + "_spliced" 
                                         for element 
                                         in assign_conversions_to_spliced]

        assign_conversions_to = \
            list(set(assign_conversions_to_retained + assign_conversions_to_spliced))
